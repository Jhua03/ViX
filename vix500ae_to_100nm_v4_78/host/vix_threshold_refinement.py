#!/usr/bin/env python3
"""V4.78 targeted threshold-refinement sweep for ViX500AE/MX80L/Pico2.

V4.78 is a supervised targeted threshold-refinement sweep. It does not try to declare a final 100 nm pass. It validates the 400 nm result from v4.77 and checks whether a small current/gain nudge can move the usable threshold toward 300/250/200 nm:

  1. Configure ViX/Pico safely while OFF.
  2. Enable once with zero PIVF gains and low CL, allowing one bounded ON kick.
  3. Apply targeted damped candidates from the v4.77 result.
  4. Re-reference the ViX at the post-kick/post-gain location.
  5. Run sham/no-step, then a denser +/-50, +/-40, +/-35, +/-30, +/-25, +/-20, +/-15, +/-10 count ladder from the already-energised state.
...
FOCUSED_CANDIDATES: tuple[tuple[str, Gains, int], ...] = (
    # v4.77 best strong threshold: 40 counts / 400 nm at CL=2.
    ("damped_gi0p2_gp2_gv1p5_cl2", Gains(0, 0.20, 2.00, 1.50, 0), 2),
    # Current-clamp nudges: still conservative, but tests whether CL=2 is the small-signal limiter.
    ("damped_gi0p2_gp2_gv1p5_cl3", Gains(0, 0.20, 2.00, 1.50, 0), 3),
    ("damped_gi0p2_gp2_gv1p5_cl4", Gains(0, 0.20, 2.00, 1.50, 0), 4),
    # Slightly stronger integral at the same damping, to test whether preload/static error is limiting 20-30 count response.
    ("damped_gi0p3_gp2_gv1p5_cl3", Gains(0, 0.30, 2.00, 1.50, 0), 3),
)
...
def make_run_dir(root: Path) -> Path:
    path = root / f"{now_stamp()}_threshold_refinement"
...
def blocks_for_amplitude(amplitude: int) -> int:
    # v4.78 is a refinement sweep: use more repeats around the 40-count
    # threshold without making the run too long.
    if amplitude >= 50:
        return 2
    if amplitude >= 35:
        return 3
...
def classify(report: dict[str, Any], args: argparse.Namespace) -> dict[str, Any]:
    ...
    if best:
        likely = "threshold_refinement_completed_usable_threshold_found"
        next_steps.append(
        ...
        likely = "threshold_refinement_completed_partial_only"
        next_steps.append(
        ...
        likely = "threshold_refinement_no_small_signal_response"
...
def run_probe(args: argparse.Namespace) -> int:
    ...
    report: dict[str, Any] = {
        ...
        "test_intent": "Targeted threshold refinement: validate 400 nm from v4.77, then test damped CL/gain nudges over 500/400/350/300/250/200/150/100 nm without claiming final 100 nm validation.",
    ...
    try:
        log.say("THRESHOLD REFINEMENT v4.78: validate 400 nm result, then test conservative CL/gain nudges over 500/400/350/300/250/200/150/100 nm. Stay present.")
        ...
        for label, gains, cl_percent in FOCUSED_CANDIDATES:
            ...
            try:
                ...
                if any(token in text for token in ("Pico latch", "fault line", "Encoder-invalid", "Movement exceeded", "operator_stop")):
                    ...
                    break
            candidate_reports.append(c)
            (run_dir / "THRESHOLD_REFINEMENT_INTERMEDIATE.json").write_text(json.dumps(candidate_reports, indent=2) + "\n", encoding="utf-8")
        ...
        (run_dir / "THRESHOLD_REFINEMENT_SUMMARY.json").write_text(json.dumps(report, indent=2) + "\n", encoding="utf-8")
        (run_dir / "THRESHOLD_REFINEMENT_VERDICT.txt").write_text("\n".join(lines), encoding="utf-8")
        log.say("\n" + "\n".join(lines))
        code = 0
    except Exception as exc:
        ...
        if "verdict" not in report:
            report["verdict"] = {
                "likely_blocker_or_status": "aborted_before_refinement_verdict",
                "passed_candidates": [],
                "findings": ["Threshold refinement stopped safely before final verdict; do not treat as a pass."],
                "next_steps": ["Inspect THRESHOLD_REFINEMENT_SUMMARY.json and per-window CSVs, then fix the abort reason before final validation."],
            }
        with contextlib.suppress(Exception):
            (run_dir / "THRESHOLD_REFINEMENT_SUMMARY.json").write_text(json.dumps(report, indent=2) + "\n", encoding="utf-8")
        log.say(f"\nABORT: Threshold refinement stopped safely: {exc}")
        code = 1
    finally:
        if pico is not None:
            with contextlib.suppress(Exception):
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="V4.78 targeted threshold-refinement motion sweep: 50/40/35/30/25/20/15/10 count ladder")
    ...
    p.add_argument("--amplitudes", default="50,40,35,30,25,20,15,10", help="Comma-separated positive count amplitudes to test; default maps 500 to 100 nm.")
